...
lede: "LFM gets its own GitHub Pages landing page — three-stage SCT diagram, asymmetric hero, manuscript-flavored type, and a package boundary that won't let the splash bleed into JSR."
publish: true
authors:
  - Michael Staton
augmented_with:
...
files_changed:
  ...
  - splash/src/components/SctDiagram.astro
  ...
  - splash/src/content/sct-examples/video-embeds.md
---
...
The splash also lets us *show* what LFM is for, not just tell. The hero centerpiece is a three-stage diagram of what we're calling the **SCT paradigm** — Syntax → Component → Trigger — with a worked example panel underneath that's collection-driven, so adding the next live example is a single markdown file under `src/content/sct-examples/`. Today's example: video embeds (bare YouTube share URL or `:::youtube-share[...]` directive → `remark-link-preview.ts` → a YouTube embed component).

## What's New?

- **`splash/` directory** at the repo root with a complete Astro site: hero, SCT diagram, curated feature gallery (7 cards), changelog list + detail, context-v list + detail (grouped by subdirectory).
- **A SCT paradigm diagram component** (`SctDiagram.astro`) with the three-stage flow, printer's-mark corner ticks, and a worked-example panel populated from a content collection.
...
└── splash/
    ...
    └── src/
        ...
        │   ├── MetaTags.astro
        │   ├── ModeToggle.astro
        │   ├── SctDiagram.astro        ← the centerpiece
        │   └── FeatureCard.astro
        ├── pages/
        │   ├── index.astro             hero + SCT + gallery + recent
        │   ├── changelog/{index, [...slug]}.astro
        │   └── context-v/{index, [...slug]}.astro
        └── content/
            ├── feature-highlights/     7 cards
            └── sct-examples/           1 today (video-embeds)
```
...
- **Asymmetric hero** — copy left, SCT diagram dominant on the right at desktop widths. The diagram is the message; the diagram gets the real estate.
...
- **Squarer card chrome** — `--radius-md: 4px` (the sibling splashes use 8–12px). Cards feel printed instead of glassy. Printer's-mark corner ticks on the SCT panel reinforce that.
...
- **More SCT examples** — callouts, citations, bare-link unfurls. Each one is a `src/content/sct-examples/<name>.md` file plus a swap of the featured flag.
