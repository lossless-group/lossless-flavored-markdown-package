...
lede: "124 assertions across every plugin, zero new dependencies — plus a demo page whose numbers are computed by the real pipeline at build time. Writing them found a bug that had been shipping for months: a callout nested inside a callout was never transformed."
publish: true
summary: "First test suite for @lossless-group/lfm — node:test + node:assert, ten files, run against dist/ so the built artifact is what gets exercised. Also adds splash/src/pages/demo.astro, which calls parseMarkdown at build time so the page doubles as an integration test. Load when adding a plugin (there is now a place to put its tests), when changing an export name (the naming contract is enforced in test/naming-contract.test.mjs), or when wondering whether a documented behaviour is actually implemented."
category: Feature
tags:
  - Testing
  - Quality
...
There are now **124 assertions across all nine plugins**, plus a demo page whose every number is computed by the real pipeline at build time. Writing them took an afternoon and found a bug that had been shipping for months.

## What's New?

- **`pnpm test`** — 124 assertions, ten files, **zero new dependencies**. `node:test` and `node:assert` are in the runtime already.
