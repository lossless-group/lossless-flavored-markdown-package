...
const sctExampleSchema = z
  .object({
    ...
    order: lenientNumber,
    featured: lenientBoolean,
    syntax_label: lenientString,
    /** Array of { kind, code } objects — flow form: [{kind: bare, code: '...'}].
        We accept any shape; the renderer is defensive. */
    syntax_examples: z.preprocess(
      (v) => (v === '' || v === null ? undefined : v),
      z.array(z.any()).optional(),
    ),
    parse_file: lenientString,
    component_file: lenientString,
    status: lenientString,
    tags: lenientStringArray,
  })
  .passthrough();
